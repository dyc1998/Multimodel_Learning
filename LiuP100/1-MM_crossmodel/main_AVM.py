...
import model_load
from train_eval_test import train_one_epoch, eval_one_epoch,train_one_epoch_MAV,eval_one_epoch_MAV
...
def main(opts, flod):
    ...
    tb_writer = SummaryWriter(
        '/home/liuzhengyu/duanw/python_data/multimodal/ThreeOnly18/txt_files/mm3s/runs/6/flod_{:d}'.format(flod))
    ...
    model_multi=multimodalcnn.MultiModalCNN(device,num_classes=2,fusion='lt_drop').to(device)
    ...
    scheduler_v = torch.optim.lr_scheduler.MultiStepLR(optimizer_a, [15, 30, 45, 60, 75, 90], gamma=0.5,
                                                       last_epoch=-1)  # 在这个节点处，将学习率乘以0.5
    scheduler_m = torch.optim.lr_scheduler.MultiStepLR(optimizer_a, [15, 30, 45, 60, 75, 90], gamma=0.5,
                                                       last_epoch=-1)  # 在这个节点处，将学习率乘以0.5
    start_epoch = 0
    if opts.resume:
        pass
    ...
    for epoch in range(start_epoch,opts.epochs):
        train_l_al,A_acc,A_recall,A_f1,V_acc,V_recall,V_f1,M_acc,M_recall,M_f1=train_one_epoch_MAV(device,train_loader,loss_f,optimizer_a,optimizer_v,optimizer_m,
                                                                                                   epoch,model_audio,model_video,model_multi)
        eval_l_al,A_acc,A_recall, A_f1, V_acc, V_recall, V_f1, M_acc, M_recall, M_f1=eval_one_epoch_MAV(device,eval_loader,loss_f,epoch,model_audio,model_video,
                                                                                                        model_multi)

        scheduler_a.step()
        scheduler_v.step()
        scheduler_m.step()
        tb_writer.add_scalar('eval_loss_al', eval_l_al, epoch)
        tb_writer.add_scalar('Ae_acc', A_acc, epoch)
        tb_writer.add_scalar('Ae_recall', A_recall, epoch)

        tb_writer.add_scalar('Ve_acc', V_acc, epoch)
        tb_writer.add_scalar('Ve_recall', V_recall, epoch)

        tb_writer.add_scalar('Me_acc', M_acc, epoch)
        tb_writer.add_scalar('Me_recall', M_recall, epoch)

        tb_writer.add_scalar('lr', optimizer_m.param_groups[0]['lr'], epoch)
        ...
        checkpoint_m = {'state_dict': model_multi.state_dict()}

        torch.save(checkpoint_a,
                   '/home/liuzhengyu/duanw/python_data/multimodal/ThreeOnly18/txt_files/mm3s/checkpoint/6/audio_flod_{}.pth'.format(flod))
        torch.save(checkpoint_v,
                   '/home/liuzhengyu/duanw/python_data/multimodal/ThreeOnly18/txt_files/mm3s/checkpoint/6/video_flod_{}.pth'.format(flod))
        torch.save(checkpoint_m,
                   '/home/liuzhengyu/duanw/python_data/multimodal/ThreeOnly18/txt_files/mm3s/checkpoint/6/multi_flod_{}.pth'.format(flod))
