...
def train_one_epoch_V(model, device, data_loader, loss_f, optimizer, epoch):
    model.train()
    scalar = GradScaler()
    ...
    for step, data in enumerate(data_loader):
        ...
        with autocast():
            pred = model(visual_inputs)[0]  # 因为代码里有这个feature
...
@torch.no_grad()
def eval_one_epoch_V(model, data_loader, device, loss_f, epoch):
    model.eval()
    accu_loss = torch.zeros(1).to(device)
    accu_num = torch.zeros(1).to(device)
    recall_pos = torch.zeros(1).to(device)  # recall所需要的分子
    ...
    for step, data in enumerate(data_loader):
        ...
        recall = recall_pos.item() / recall_num.item()
        f1 = F1_score(precision, recall)
        data_loader.desc = "[eval_epoch {}]  loss:{:.3f}, acc:{:.3f},pre:{:.3f}, recall:{:.3f},  F1-score:{:.3f}".format(epoch, l, acc, precision, recall, f1)
    return l, acc, precision, recall, f1